...
import { Calendar, Clock, Stethoscope, Trash2, CalendarCheck, Loader2 } from "lucide-react";
...
export default function MyAppointments() {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || "Could not fetch appointments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    const confirmed = window.confirm("Are you sure you want to cancel/delete this appointment?");
    if (!confirmed) return;
        ...
        <div className="flex-1 min-w-0">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm">
              ...
              <h3 className="text-lg font-bold text-gray-800">No Appointments Booked</h3>
              <p className="text-sm text-gray-500 mt-1 max-w-md mx-auto">
                You haven't scheduled any doctor appointments yet. You can book an appointment directly from your dashboard.
              </p>
            </div>
          ) : (
            <div className="grid gap-4">
              {appointments.map((appt) => (
                <div
                  key={appt.id}
                  className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 border border-gray-200/80 p-5 sm:p-6"
                >
